...
from SwinTransformerV2_padding_mask_modified_ape_rpb import SwinTransformerV2
...
class MyDatasets(torch.utils.data.Dataset):
    def __init__(self, mode):
        ...
        traindir = os.path.join("F:\\face\\imagenet", 'train')     # /train/ を指定されたパスに追加
        valdir = os.path.join("F:\\face\\imagenet", 'validation_folder')
...
model = SwinTransformerV2( ape = True )
...
PATH = 'model_SWTRFV2_padding_mask_ape_rpb_best.pth'
...
f_train = open( "P_M_ape_rpb_train.log", mode="w", encoding = "UTF-8" )
f_train.close()
f_val = open( "P_M_ape_rpb_val.log", mode="w", encoding = "UTF-8" )
f_val.close()
...
for epoch in range(epoch_begin, config.num_epochs):
    ...
    # 検証セットを使って精度評価
    val_loss, val_accuracy, global_val_step = eval.evaluate3(
    ...
    if val_loss < val_loss_best:
        ...
        torch.save(
            ...
            'loss': loss,},
            f'model_SWTRFV2_padding_mask_ape_rpb_best.pth')
...
torch.save(
    ...
    'optimizer_state_dict': optimizer.state_dict(),
    'loss': loss,},
    f'model_SWTRFV2_padding_mask_ape_rpb.pth')
